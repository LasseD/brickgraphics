...
public class PeeronColorSheetParser {
		...
		while(scanner.hasNextLine()) {
			...
			if(line.equals("<tr bgcolor=\"#dddddd\">") || 
			   line.equals("<tr bgcolor=\"#eeeeee\">")) {
				...
				if(newColor != null) {
					out.put(newColor.getNamesPeeron(), newColor);					
				}
				else {
	...
	private static LEGOColor parseColorLine(String line) {
		...
		try {
			...
			if(s.length() != 0)
				c.id = Integer.parseInt(s);
			
			// </td><td>Black =name LEGO
			if(!scanner.hasNext()) return null; scanner.next(); // td
			if(!scanner.hasNext()) return null; 
			c.namesLEGO = scanner.next(); // name LEGO
			...
			if(s.length() != 0) {
				if(c.id == LEGOColor.BLACK.id)
					c.setRGB(Color.BLACK);
				else if(c.id == LEGOColor.WHITE.id)
					c.setRGB(Color.WHITE);
				else
					c.setRGB(new Color(Integer.parseInt(s, 16)));
